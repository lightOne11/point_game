...
class point_game():
    ...
    def start(self,way,way_list):
        for i in sorted(range(1,self.player_n+1),reverse=True):
                    ...
                    self.place[x-1][y-1] = str(i)+'P'
                    #print(f'위치 선택 후 위치 점수 : {self.place[x][y]}\n플레이어번호 : {i}')

    def move(self,way,way_list,qi):
        self.turn += 1
        print(f'-----{self.turn} round ------')
        for i in range(1,self.player_n+1):
            if way == 'x' and i == qi: continue
            else :
                # if self.turn > 2 :
                #     if self.player[f'{i} player'][1][-1] - self.player[f'{i} player'][1][-2] == [-1,0]: 
                #     elif way == '4' : self.place[x-1][y-1] = '<'
                #     elif way == '6' : self.place[x-1][y-1] = '>'
                #     elif way == '2' : self.place[x-1][y-1] = 'v'
                while True:
                    self.display()
                ...
                if way == 'x': break
                x,y = self.player[f'{i} player'][1][-1]
                self.player[f'{i} player'][0] +=int(self.place[x-1][y-1])
                self.place[x-1][y-1] = str(i)+'P'
                way_list[x,y] = str(i)+'P'
